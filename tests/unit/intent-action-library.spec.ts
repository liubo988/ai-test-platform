...
describe('intent-action-library', () => {
  ...
  it('surfaces a dedicated primary-record resolution capability for list verification with detail fallback', () => {
    ...
    expect(capability?.implementationNotes.join('\n')).toContain("const resolvedBusinessId = businessId || ((rowText.match(/\\b\\d{6,12}\\b/g) || []).find((item) => !/^1\\d{10}$/.test(item)) || '')");
    ...
    expect(capability?.example).toContain("const resolvedBusinessId = businessId || ((rowText.match(/\\b\\d{6,12}\\b/g) || []).find((item) => !/^1\\d{10}$/.test(item)) || '');");
    expect(capability?.example).toContain("if (/新入库/.test(rowText)) expect(rowText).toContain('新入库');");
    expect(capability?.example).not.toContain("await expect(recordCheck.row).toContainText('新入库');");
    expect(capability?.example).toContain("else if (expectedStatus) expect(expectedStatus).toContain('新入库');");
