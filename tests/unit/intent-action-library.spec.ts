...
describe('intent-action-library', () => {
  ...
  it('surfaces a dedicated primary-record resolution capability for list verification with detail fallback', () => {
    ...
    expect(capability?.example).toContain("const resolvedExpectedStatus = expectedStatus || (matchedRecordByResolvedBusinessId ? __e2e.pickJsonValue(matchedRecordByResolvedBusinessId, { label: '状态'");
