...
import { buildIntentE2EReleaseStatusReport, renderIntentE2EReleaseStatusMarkdown } from '@/lib/intent-e2e-release-status';
...
describe('intent e2e release status', () => {
  ...
  it('renders a CI-friendly markdown summary without recomputing release state', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'intent-release-status-markdown-'));
    try {
      const fixture = writeStatusFixture(tempDir);
      const report = await buildIntentE2EReleaseStatusReport({
        releaseGuardConfigPath: fixture.releaseGuardConfigPath,
        knowledgeHitConfigPath: fixture.knowledgeHitConfigPath,
        releaseGuardReportPath: fixture.releaseReportPath,
        generatedAt: '2026-04-28T10:30:00.000Z',
      });

      const markdown = renderIntentE2EReleaseStatusMarkdown(report, {
        title: 'Intent E2E Release Readiness (CI static evidence)',
        generatedBy: 'unit-test',
      });

      expect(markdown).toContain('# Intent E2E Release Readiness (CI static evidence)');
      expect(markdown).toContain('- Status: `ready` (可发布)');
      expect(markdown).toContain('- Can release: `yes`');
      expect(markdown).toContain('- Readiness summary: release guard、knowledge-hit 与最近 compare 证据当前齐全。');
      expect(markdown).toContain('| Release guard assets | passed (通过) | yes |');
      expect(markdown).toContain('| list_search_detail | passed (通过; 3 runs, terminal 100%, first-pass 100%) |');
      expect(markdown).toContain('Generated by: `unit-test`');
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
  });

  it('marks skipped current compare summaries as static evidence only', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'intent-release-status-static-summary-'));
    try {
      const fixture = writeStatusFixture(tempDir);
      const report = await buildIntentE2EReleaseStatusReport({
        releaseGuardConfigPath: fixture.releaseGuardConfigPath,
      ...
      expect(markdown).toContain('This is a static evidence summary.');
      expect(markdown).toContain('| list_search_detail | skipped (compare not loaded) |');
      expect(report.status).toBe('attention');
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
