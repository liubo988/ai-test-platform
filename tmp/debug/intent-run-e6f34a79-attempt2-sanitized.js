test("在订单列表通过展开筛选将入账状态过滤为“待申请”，勾选结果行后使用表头“批量入账”提交“批量申请入账”弹窗，并跳转到入账管理按订单号检索，校验订单号、服务项、入", async ({ page }) => {
      ...
      for (let txtIndex = 0; txtIndex < txtSourceCount; txtIndex += 1) {
        ...
        for (let txtPartIndex = 0; txtPartIndex < txtPartSourceCount; txtPartIndex += 1) {
          ...
          for (let txtPartPartIndex = 0; txtPartPartIndex < txtPartPartSourceCount; txtPartPartIndex += 1) {
            ...
            const txtPartPartPartRowKey = ((await txtPartPartSource.getAttribute('data-row-key')) || '').trim();
            const txtPartPartPartSources = txtPartPartPartRowKey ? page.locator(`tr[data-row-key="${txtPartPartPartRowKey}"]`) : txtPartPartSource;
            const txtPartPartPartParts = [];
            const txtPartPartPartSourceCount = txtPartPartPartRowKey ? await txtPartPartPartSources.count() : 1;
            for (let txtPartPartPartIndex = 0; txtPartPartPartIndex < txtPartPartPartSourceCount; txtPartPartPartIndex += 1) {
              const txtPartPartPartSource = txtPartPartPartRowKey ? txtPartPartPartSources.nth(txtPartPartPartIndex) : txtPartPartSource;
              const txtPartPartPartPart = (await txtPartPartPartSource.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();
              if (txtPartPartPartPart && !txtPartPartPartParts.includes(txtPartPartPartPart)) txtPartPartPartParts.push(txtPartPartPartPart);
            }
            const txtPartPartPart = txtPartPartPartParts.join(' ').trim();
  ...
  await test.step("Step 4: 提取弹窗中的关键对账信息", async () => {
    ...
    if (!shared.selectedAmount) throw new Error('提取失败：批量申请入账弹窗未读取到入账金额');
  ...
  await test.step("Step 8: 校验入账记录字段一致", async () => {
    ...
    {
      const selectedAmountText = String(shared.selectedAmount || '').trim();
      const normalizedSelectedAmount = selectedAmountText.replace(/,/g, '');
      const selectedAmountLooksLikeDate = /^(?:19|20)\d{2}[-/.](?:0?[1-9]|1[0-2])[-/.](?:0?[1-9]|[12]\d|3[01])$/.test(selectedAmountText) || /^(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])$/.test(normalizedSelectedAmount);
      const selectedAmountLooksLikeLongId = !normalizedSelectedAmount.includes('.') && normalizedSelectedAmount.length >= 10;
      const shouldAssertSelectedAmount = /^\d+(?:\.\d{1,2})?$/.test(normalizedSelectedAmount) && !selectedAmountLooksLikeDate && !selectedAmountLooksLikeLongId && Number(normalizedSelectedAmount) > 0;
      if (shouldAssertSelectedAmount) {
        expect(rowText.replace(/,/g, '')).toContain(shared.selectedAmount);
      } else if (selectedAmountText) {
        artifacts['selectedAmount_assertion_skipped'] = selectedAmountText;
      }
    }
  ...
  await test.step("Verification: 最终业务验收", async () => {
    ...
    }
    {
      ...
      const shouldAssertSelectedAmount = /^\d+(?:\.\d{1,2})?$/.test(normalizedSelectedAmount) && !selectedAmountLooksLikeDate && !selectedAmountLooksLikeLongId && Number(normalizedSelectedAmount) > 0;
      if (shouldAssertSelectedAmount) {
        expect(finalText.replace(/,/g, '')).toContain(shared.selectedAmount);
      } else if (selectedAmountText) {
        artifacts['selectedAmount_assertion_skipped'] = selectedAmountText;
      }
    }
