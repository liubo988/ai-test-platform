...
const modal = await __e2e.waitForVisibleAntdModal(page, { titleIncludes: '批量申请入账' });
    ...
    if (!normalizedModalAmount && modalAmountSourceRow) {
      ...
      for (let rowAmountIndex = 0; rowAmountIndex < rowAmountSourceCount; rowAmountIndex += 1) {
        ...
        for (let rowAmountPartIndex = 0; rowAmountPartIndex < rowAmountPartSourceCount; rowAmountPartIndex += 1) {
          ...
          const rowAmountPartPartRowKey = ((await rowAmountPartSource.getAttribute('data-row-key')) || '').trim();

          const rowAmountPartPartSources = rowAmountPartPartRowKey ? page.locator(`tr[data-row-key="${rowAmountPartPartRowKey}"]`) : rowAmountPartSource;

          const rowAmountPartPartParts = [];

          const rowAmountPartPartSourceCount = rowAmountPartPartRowKey ? await rowAmountPartPartSources.count() : 1;

          for (let rowAmountPartPartIndex = 0; rowAmountPartPartIndex < rowAmountPartPartSourceCount; rowAmountPartPartIndex += 1) {

            const rowAmountPartPartSource = rowAmountPartPartRowKey ? rowAmountPartPartSources.nth(rowAmountPartPartIndex) : rowAmountPartSource;

            const rowAmountPartPartPart = (await rowAmountPartPartSource.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();

            if (rowAmountPartPartPart && !rowAmountPartPartParts.includes(rowAmountPartPartPart)) rowAmountPartPartParts.push(rowAmountPartPartPart);

          }

          const rowAmountPartPart = rowAmountPartPartParts.join(' ').trim();
  ...
  await test.step("Step 4: 提取弹窗中的关键对账信息", async () => {
    ...
    shared.selectedOrderNo = orderNoMatch ? orderNoMatch[1].trim() : '';
    {
      ...
      {
        const selectedServiceItemCandidateText = String(selectedServiceItemCandidate && !selectedServiceItemLooksLikeStatus ? selectedServiceItemCandidate : '' || '' || '').trim();
