  ...
  await test.step("Step 3: 勾选订单并提取对账字段", async () => {
    ...
    for (let rowTextIndex = 0; rowTextIndex < rowTextSourceCount; rowTextIndex += 1) {
      ...
      for (let rowTextPartIndex = 0; rowTextPartIndex < rowTextPartSourceCount; rowTextPartIndex += 1) {
        ...
        const rowTextPartPartRowKey = ((await rowTextPartSource.getAttribute('data-row-key')) || '').trim();
        const rowTextPartPartSources = rowTextPartPartRowKey ? page.locator(`tr[data-row-key="${rowTextPartPartRowKey}"]`) : rowTextPartSource;
        const rowTextPartPartParts = [];
        const rowTextPartPartSourceCount = rowTextPartPartRowKey ? await rowTextPartPartSources.count() : 1;
        for (let rowTextPartPartIndex = 0; rowTextPartPartIndex < rowTextPartPartSourceCount; rowTextPartPartIndex += 1) {
          const rowTextPartPartSource = rowTextPartPartRowKey ? rowTextPartPartSources.nth(rowTextPartPartIndex) : rowTextPartSource;
          const rowTextPartPartPart = (await rowTextPartPartSource.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();
          if (rowTextPartPartPart && !rowTextPartPartParts.includes(rowTextPartPartPart)) rowTextPartPartParts.push(rowTextPartPartPart);
        }
        const rowTextPartPart = rowTextPartPartParts.join(' ').trim();
...
const modal = await __e2e.waitForVisibleAntdModal(page, { titleIncludes: '批量申请入账' });
  ...
  await test.step("Step 5: 校验弹窗默认带出信息", async () => {
    ...
    for (let modalTextIndex = 0; modalTextIndex < modalTextSourceCount; modalTextIndex += 1) {
      ...
      for (let modalTextPartIndex = 0; modalTextPartIndex < modalTextPartSourceCount; modalTextPartIndex += 1) {
        const modalTextPartSource = modalTextPartRowKey ? modalTextPartSources.nth(modalTextPartIndex) : modalTextSource;
        const modalTextPartPartRowKey = ((await modalTextPartSource.getAttribute('data-row-key')) || '').trim();
        const modalTextPartPartSources = modalTextPartPartRowKey ? page.locator(`tr[data-row-key="${modalTextPartPartRowKey}"]`) : modalTextPartSource;
        const modalTextPartPartParts = [];
        const modalTextPartPartSourceCount = modalTextPartPartRowKey ? await modalTextPartPartSources.count() : 1;
        for (let modalTextPartPartIndex = 0; modalTextPartPartIndex < modalTextPartPartSourceCount; modalTextPartPartIndex += 1) {
          const modalTextPartPartSource = modalTextPartPartRowKey ? modalTextPartPartSources.nth(modalTextPartPartIndex) : modalTextPartSource;
          const modalTextPartPartPart = (await modalTextPartPartSource.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();
          if (modalTextPartPartPart && !modalTextPartPartParts.includes(modalTextPartPartPart)) modalTextPartPartParts.push(modalTextPartPartPart);
        }
        const modalTextPartPart = modalTextPartPartParts.join(' ').trim();
        if (modalTextPartPart && !modalTextPartParts.includes(modalTextPartPart)) modalTextPartParts.push(modalTextPartPart);
      }
      const modalTextPart = modalTextPartParts.join(' ').trim();
      if (modalTextPart && !modalTextParts.includes(modalTextPart)) modalTextParts.push(modalTextPart);
  ...
  await test.step("Verification: 最终业务验收", async () => {
    ...
    for (let finalRowTextIndex = 0; finalRowTextIndex < finalRowTextSourceCount; finalRowTextIndex += 1) {
      ...
      for (let finalRowTextPartIndex = 0; finalRowTextPartIndex < finalRowTextPartSourceCount; finalRowTextPartIndex += 1) {
        ...
        const finalRowTextPartPartRowKey = ((await finalRowTextPartSource.getAttribute('data-row-key')) || '').trim();
        const finalRowTextPartPartSources = finalRowTextPartPartRowKey ? page.locator(`tr[data-row-key="${finalRowTextPartPartRowKey}"]`) : finalRowTextPartSource;
        const finalRowTextPartPartParts = [];
        const finalRowTextPartPartSourceCount = finalRowTextPartPartRowKey ? await finalRowTextPartPartSources.count() : 1;
        for (let finalRowTextPartPartIndex = 0; finalRowTextPartPartIndex < finalRowTextPartPartSourceCount; finalRowTextPartPartIndex += 1) {
          const finalRowTextPartPartSource = finalRowTextPartPartRowKey ? finalRowTextPartPartSources.nth(finalRowTextPartPartIndex) : finalRowTextPartSource;
          const finalRowTextPartPartPart = (await finalRowTextPartPartSource.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();
          if (finalRowTextPartPartPart && !finalRowTextPartPartParts.includes(finalRowTextPartPartPart)) finalRowTextPartPartParts.push(finalRowTextPartPartPart);
        }
        const finalRowTextPartPart = finalRowTextPartPartParts.join(' ').trim();
