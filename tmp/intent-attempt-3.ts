test('商机业务流：新建商机并在我创建的列表校验新入库', async ({ page }) => {
  ...
  await expect(page.locator('.ant-form-explain, .ant-form-item-explain-error').filter({ hasText: /必填|请选择|请输入/ })).toHaveCount(0);
  ...
  await __e2e.observeSubmitState(page, {
    ...
    successLocator: page.getByPlaceholder('商机ID/联系人名称/电话/企业名称').first(),
  });
