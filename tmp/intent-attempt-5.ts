test('商机业务流：新建商机并在我创建的列表校验新入库', async ({ page }) => {
  ...
  await expect(newBusinessBtn).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.getByRole('heading', { name: '关联产品意向信息' }).first()).toBeVisible({ timeout: 20000 });
  await expect(page.locator('.ant-form-item-explain-error').filter({ hasText: /必填|请选择|请输入/ })).toHaveCount(0);
  ...
  await expect(companyRow).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.getByText('附件信息').first()).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.getByText('我创建的', { exact: true }).first()).toBeVisible({ timeout: 10000 });

  // Step 6: 校验新建记录与状态
  const visibleKeywordInput = page.locator('input#businessList_keywords[placeholder="商机ID/联系人名称/电话/企业名称"]:visible').first();
  await expect(visibleKeywordInput).toBeVisible({ timeout: 20000 });
  await visibleKeywordInput.fill(leadMobile);
