test('商机业务流：新建商机并在我创建的列表校验新入库', async ({ page }) => {
  ...
  const leadMobile = `1990000${stamp.slice(-4)}`; // 无空格，规避历史手机号空格问题
  const leadSource = '抖音';
  const leadGender = '男';
  ...
  await mobileInput.fill(leadMobile.replace(/\s+/g, ''));
  ...
  await expect(page.getByText('关联产品意向信息').first()).toBeVisible({ timeout: 15000 });
  ...
  const step2Resp = __e2e.waitForApiResponse(page, { urlIncludes: '/business', method: 'POST' }).catch(() => null);
