...
function pickJsonValue(source, options) {
  ...
  if (!normalizedOptions.required) {
    emitLog('warn', 'json value not found', {
      label: normalizedOptions.label || null,
      paths,
...
function buildPrimaryLookupInputCandidates(page, options) {
  return [
    ...
    page.getByPlaceholder(
      /商机ID\/联系人名称\/电话\/企业名称|订单ID\/联系人名称\/电话\/企业名称|businessId|orderId|商机ID|订单ID|联系人名称|企业名称|手机号|关键字|关键词|搜索|查询/i
    ).first(),
...
async function resolvePrimaryRecord(page, options) {
  ...
  for (let attemptIndex = 1; attemptIndex <= maxLookupAttempts; attemptIndex += 1) {
    ...
    const keywordInput = await pickVisibleLocator(
      buildPrimaryLookupInputCandidates(page, options),
      Math.min(remainingTime(), Math.max(300, Number(options?.inputTimeoutMs || 1200)))
    );
    if (!keywordInput) {
      throw new Error(`未找到可见列表检索框：primaryValue=${primaryValue}`);
    }
    ...
    .evaluate(
      (node, payload) => {
        ...
        const toValueText = (element, labelText) => {
          ...
          if (stripped && stripped !== normalize(labelText)) return stripped;
          if (directText !== normalize(labelText)) return directText;
          return '';
        };
        ...
        for (const candidate of candidates.slice(0, 240)) {
          ...
          for (const labelNode of labelNodes) {
            ...
            const fallbackText = stripLabelPrefix(candidateText, labelText);
        ...
        if (rootInlineMatch?.[1]) {
          return {
            labelText: targetLabel,
            valueText: normalize(rootInlineMatch[1]),
...
async function readDetailField(page, options) {
  ...
  while (Date.now() < deadline) {
    ...
    if (isLocatorLike(options?.scope)) {
      ...
      if (detailLayer) scopes.push(detailLayer);
    }
