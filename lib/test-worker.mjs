...
const WORKER_GOTO_TIMEOUT_FALLBACK_MS = 30_000;
const WORKER_GOTO_RECOVERY_TIMEOUT_MS = 8_000;
const WORKER_RETRYABLE_GOTO_PATTERNS = [/net::ERR_ABORTED/i, /Timeout \d+ms exceeded/i];

function getGotoTimeoutMs(options) {
  const timeout = Number(options?.timeout);
  return Number.isFinite(timeout) && timeout > 0 ? timeout : WORKER_GOTO_TIMEOUT_FALLBACK_MS;
}

function parseWorkerUrl(value, baseUrl) {
  try {
...
function normalizeWorkerUrl(value) {
  const parsed = parseWorkerUrl(value);
  if (!parsed) return String(value || '').replace(/\/+$/, '');
  return `${parsed.origin}${parsed.pathname}${parsed.search}${parsed.hash}`.replace(/\/+$/, '');
}

function sameWorkerDocumentRoute(left, right) {
  return Boolean(
    left &&
      right &&
...
function hasWorkerHashRoute(url) {
  return Boolean(url?.hash && /^#\//.test(url.hash));
}

function extractInterruptedNavigationUrls(error) {
  const message = toErrorMessage(error);
  const match = message.match(/Navigation to ["']([^"']+)["'] is interrupted by another navigation to ["']([^"']+)["']/i);
  if (!match) return null;
  return {
    targetUrl: match[1],
    interruptedByUrl: match[2],
  };
}

function isRecoverableSameDocumentHashNavigationInterruption(error, requestedUrl) {
  const interrupted = extractInterruptedNavigationUrls(error);
  if (!interrupted && !/interrupted by another navigation/i.test(toErrorMessage(error))) {
    return false;
  }

  const target = parseWorkerUrl(interrupted?.targetUrl || requestedUrl);
  const interruptedBy = parseWorkerUrl(interrupted?.interruptedByUrl || '');

  if (!target || !hasWorkerHashRoute(target)) return false;
  if (interruptedBy) return sameWorkerDocumentRoute(target, interruptedBy);
  return true;
}

function isRetryableWorkerGotoError(error, requestedUrl) {
  const message = toErrorMessage(error);
  return (
    WORKER_RETRYABLE_GOTO_PATTERNS.some((pattern) => pattern.test(message)) ||
    isRecoverableSameDocumentHashNavigationInterruption(error, requestedUrl)
  );
}

async function waitForWorkerGotoSettle(page, timeoutMs) {
  const boundedTimeoutMs = Math.min(Math.max(1000, Number(timeoutMs) || WORKER_GOTO_RECOVERY_TIMEOUT_MS), 10_000);
  await page.waitForLoadState('domcontentloaded', { timeout: boundedTimeoutMs }).catch(() => {});
  await page.waitForTimeout(250).catch(() => {});
}

async function recoverSameDocumentHashNavigation(page, targetUrl, options) {
  const target = parseWorkerUrl(targetUrl, page.url());
  if (!target || !hasWorkerHashRoute(target)) {
    return { recovered: false, restored: false, finalUrl: page.url() };
  }

  const timeoutMs = Math.min(getGotoTimeoutMs(options), WORKER_GOTO_RECOVERY_TIMEOUT_MS);
  await waitForWorkerGotoSettle(page, timeoutMs);

  const beforeUrl = page.url();
  if (normalizeWorkerUrl(beforeUrl) === normalizeWorkerUrl(target.href)) {
    return { recovered: true, restored: true, finalUrl: beforeUrl };
  }

  const current = parseWorkerUrl(beforeUrl, target.href);
  if (!sameWorkerDocumentRoute(target, current)) {
    return { recovered: false, restored: false, finalUrl: beforeUrl };
  }

  await page
    .evaluate((nextUrl) => {
      const targetLocation = new URL(nextUrl, window.location.href);
      if (window.location.href !== targetLocation.href) {
  ...
  await page
    .waitForURL((url) => normalizeWorkerUrl(url.toString()) === normalizeWorkerUrl(target.href), { timeout: timeoutMs })
    .catch(() => {});
  await waitForWorkerGotoSettle(page, timeoutMs);

  const finalUrl = page.url();
  return {
    recovered: sameWorkerDocumentRoute(target, parseWorkerUrl(finalUrl, target.href)),
    restored: normalizeWorkerUrl(finalUrl) === normalizeWorkerUrl(target.href),
    finalUrl,
  };
}

async function resilientPageGoto(page, originalGoto, url, options) {
  try {
    return await originalGoto(url, options);
  } catch (error) {
    if (!isRetryableWorkerGotoError(error, url)) {
      throw error;
    }

    const interrupted = extractInterruptedNavigationUrls(error);
    const targetUrl = interrupted?.targetUrl || String(url || '');
    const hashRecovery = await recoverSameDocumentHashNavigation(page, targetUrl, options);
    if (hashRecovery.recovered) {
      emitLog('warn', 'page.goto recovered after SPA hash navigation interruption', {
        targetUrl,
        finalUrl: hashRecovery.finalUrl,
        restored: hashRecovery.restored,
    ...
    try {
      const response = await originalGoto(url, retryOptions);
      await waitForWorkerGotoSettle(page, getGotoTimeoutMs(options));
      emitLog('warn', 'page.goto recovered with commit retry', {
      ...
      const retryInterrupted = extractInterruptedNavigationUrls(retryError);
      const retryTargetUrl = retryInterrupted?.targetUrl || targetUrl;
      const retryHashRecovery = await recoverSameDocumentHashNavigation(page, retryTargetUrl, options);
      if (retryHashRecovery.recovered) {
        emitLog('warn', 'page.goto recovered after commit retry SPA hash interruption', {
          targetUrl: retryTargetUrl,
          finalUrl: retryHashRecovery.finalUrl,
          restored: retryHashRecovery.restored,
          originalError: toErrorMessage(error),
...
function installResilientGoto(page) {
  if (!page || page.__intentE2eResilientGotoInstalled) return;
  const originalGoto = page.goto.bind(page);
  Object.defineProperty(page, '__intentE2eResilientGotoInstalled', {
    value: true,
    enumerable: false,
    configurable: false,
  });
  page.goto = async (url, options) => resilientPageGoto(page, originalGoto, url, options);
}
...
async function resolveLoginSubmitButton(page) {
  const exactSubmit = page.getByRole('button', { name: /^(?:登\s*录|登录|Login|Sign in)$/i }).first();
  if (await locatorVisible(exactSubmit, 1000)) return exactSubmit;

  const nonModeSubmit = page
    .getByRole('button', { name: loginButtonNamePattern })
    .filter({ hasNotText: /企业微信登录|管帮手登录|短信验证码登录|短信登录|验证码登录|密码登录|扫码登录|二维码登录|QR/i })
    .first();
  if (await locatorVisible(nonModeSubmit, 1000)) return nonModeSubmit;

  return page.getByRole('button', { name: loginButtonNamePattern }).last();
}
...
async function waitForLikelyLoginPage(page, timeoutMs) {
...
async function ensureLoginSurface(page, auth, options) {
  ...
  if (shouldOpenConfiguredLoginUrl(currentPageLooksLikeLogin, auth.loginUrl)) {
    ...
    if (await waitForLikelyLoginPage(page, Number(options?.loginDetectionTimeoutMs || 3000))) return true;
...
async function loginWithEnvAuth(page, options) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const loginButton = await resolveLoginSubmitButton(page);
...
async function ensureLoggedIn(page, options) {
  ...
  if (!(await waitForLikelyLoginPage(page, Number(options?.loginDetectionTimeoutMs || 3000)))) {
    return false;
  }
...
async function findAntdFormItemByLabel(scope, label, options) {
  const normalizedLabel = normalizeVisibleText(label || '');
  if (!normalizedLabel) throw new Error('findAntdFormItemByLabel 需要提供 label');

  const timeoutMs = Math.max(200, Number(options?.timeoutMs || 5000));
  const required = options?.required !== false;
  const exactText = new RegExp(`^\\s*${escapeRegExp(normalizedLabel)}\\s*$`);
  const escapedLabel = escapeAttributeValue(normalizedLabel);
  const deadline = Date.now() + timeoutMs;

  while (Date.now() < deadline) {
    const labelCandidates = [
      scope.locator(`.ant-form-item-label label[title="${escapedLabel}"]`),
      scope.locator(`label[title="${escapedLabel}"]`),
      scope.locator('.ant-form-item-label label').filter({ hasText: exactText }),
      scope.locator('label').filter({ hasText: exactText }),
      scope.locator('.ant-form-item-label').filter({ hasText: exactText }),
    ];

    for (const group of labelCandidates) {
      const count = Math.min(await group.count().catch(() => 0), 8);
      for (let index = 0; index < count; index += 1) {
        const labelLocator = group.nth(index);
        if (!(await locatorVisible(labelLocator, 120))) continue;
        const formItem = labelLocator.locator('xpath=ancestor::*[contains(concat(" ", normalize-space(@class), " "), " ant-form-item ")][1]').first();
        if (!(await locatorVisible(formItem, 120))) continue;
        const labelText = normalizeVisibleText(await labelLocator.innerText().catch(() => ''));
        if (labelText && labelText !== normalizedLabel && !exactText.test(labelText)) continue;
        emitLog('info', 'ant-form-item resolved by exact label', {
  ...
  if (!required) return null;
  throw new Error(`未找到 Ant Design 表单项: label=${normalizedLabel}`);
}
...
async function waitForMediaPlaybackEvidence(page, options) {
  const timeoutMs = Number(options?.timeoutMs || 15000);
  const mediaSelector = String(options?.mediaSelector || 'audio, video').trim();
  const surfaceSelector = String(
    options?.surfaceSelector || '.ant-modal, .ant-drawer, .ant-modal-content, .ant-drawer-content'
  ).trim();
  const resourcePattern = String(
    options?.resourcePattern || String.raw`(?:recording\.yikaiye\.net|audioUrl|\.wav(?:[?#].*)?$|\.mp3(?:[?#].*)?$|\.m4a(?:[?#].*)?$|\.aac(?:[?#].*)?$|\.ogg(?:[?#].*)?$|\.mp4(?:[?#].*)?$|\.webm(?:[?#].*)?$)`
  );

  const handle = await page.waitForFunction(
    ({ mediaSelector: selector, surfaceSelector: surface, resourcePattern: pattern }) => {
      const normalize = (value) => String(value || '').trim();
      const queryAll = (root, rawSelector) => {
        const normalizedSelector = normalize(rawSelector).replace(/:visible/g, '');
      ...
      const resourceRegExp = new RegExp(pattern, 'i');
      const roots = [document];
      const seen = new Set();

      if (surface) {
        for (const node of queryAll(document, surface)) {
      ...
      for (const node of mediaNodes) {
        ...
        const src = normalize(node.currentSrc || node.src || node.getAttribute('src') || sourceNode?.src || sourceNode?.getAttribute('src'));
        if (!src || !resourceRegExp.test(src)) continue;
        const tagName = normalize(node.tagName).toLowerCase();
        return {
          tagName,
          src,
          currentSrc: normalize(node.currentSrc),
          readyState: typeof node.readyState === 'number' ? node.readyState : null,
          paused: typeof node.paused === 'boolean' ? node.paused : null,
          hasControls: Boolean(node.controls),
          mediaCount: mediaNodes.length,
          surfaceCount: surface ? queryAll(document, surface).length : 0,
        };
      }

      return null;
    },
    { mediaSelector, surfaceSelector, resourcePattern },
    { timeout: timeoutMs }
  );

  const evidence = await handle.jsonValue();
  emitLog('info', 'media playback evidence matched', evidence || {});
  return evidence;
}
...
async function findVisibleAntdDatePickerPanel(page) {
  ...
  for (let index = count - 1; index >= 0; index -= 1) {
    ...
    if (await locatorVisible(panel, 180)) return panel;
  }
  return null;
}

async function waitForVisibleAntdDatePickerPanel(page, timeoutMs) {
  const deadline = Date.now() + (timeoutMs || 1200);
  while (Date.now() < deadline) {
    const panel = await findVisibleAntdDatePickerPanel(page);
...
async function waitForAntdDatePickerPanelToClose(page, timeoutMs) {
...
async function setInputValueWithEvents(locator, value) {
  return locator
    .evaluate((node, nextValue) => {
      ...
      if (!target) return { ok: false, value: '' };
...
function extractDateParts(value) {
  ...
  return {
    ...
    paddedCn: `${year}年${pad(month)}月${pad(day)}日`,
  };
}

function dateInputMatchesTarget(currentValue, targetValue, options) {
  const current = normalizeVisibleText(currentValue);
  const target = normalizeVisibleText(targetValue);
  if (!current || !target) return false;

  const currentDate = extractDateParts(current);
  const targetDate = extractDateParts(target);
  if (currentDate && targetDate) {
    if (currentDate.iso !== targetDate.iso) return false;
    if (options?.requireTime === true && /\d{1,2}:\d{2}/.test(target)) {
      const currentTime = current.match(/(\d{1,2}:\d{2})(?::\d{2})?/);
      const targetTime = target.match(/(\d{1,2}:\d{2})(?::\d{2})?/);
      return Boolean(currentTime?.[1] && targetTime?.[1] && currentTime[1] === targetTime[1]);
    }
    return true;
  }
...
async function isInputReadonlyOrDisabled(locator) {
  return locator
    .evaluate((node) => {
      const target = node instanceof HTMLInputElement || node instanceof HTMLTextAreaElement ? node : null;
      if (!target) return true;
      return Boolean(target.readOnly || target.disabled || target.getAttribute('aria-disabled') === 'true');
    })
    .catch(() => true);
}

function buildAntdDatePickerPopupInputCandidates(panel) {
  return [
    panel.locator('input.ant-calendar-input:visible').first(),
    panel.locator('.ant-calendar-input-wrap input:visible').first(),
    panel.locator('.ant-picker-input input:visible').first(),
    panel.locator('input:visible').first(),
  ];
}

function buildAntdDatePickerOkButtonCandidates(panel) {
  return [
    panel.locator('.ant-calendar-ok-btn:visible').first(),
    panel.locator('.ant-picker-ok button:visible').first(),
    panel.getByRole('button', { name: /^确\s*定$|^OK$/i }).first(),
    panel.locator('button:visible').filter({ hasText: /^确\s*定$|^OK$/i }).first(),
  ];
}

async function clickAntdDatePickerOkIfPresent(page, panel, timeoutMs) {
  const deadline = Date.now() + Math.max(200, Number(timeoutMs || 1000));
  while (Date.now() < deadline) {
    const candidates = buildAntdDatePickerOkButtonCandidates(panel);
    for (const button of candidates) {
      const count = await button.count().catch(() => 0);
      if (!count || !(await locatorVisible(button, 120))) continue;
      await button.click({ force: true }).catch(() => {});
      await waitForAntdDatePickerPanelToClose(page, 1000).catch(() => false);
      return true;
    }
    await page.waitForTimeout(80);
  }
...
function buildAntdDateCellCandidates(panel, targetDate) {
  const titleCandidates = [targetDate.iso, targetDate.slash, targetDate.cn, targetDate.paddedCn];
  const candidates = [];
  for (const title of titleCandidates) {
    const escapedTitle = escapeAttributeValue(title);
    candidates.push(panel.locator(`td[title="${escapedTitle}"]:not(.ant-calendar-disabled-cell) .ant-calendar-date`).first());
    candidates.push(panel.locator(`td[title="${escapedTitle}"]:not(.ant-calendar-disabled-cell)`).first());
    candidates.push(panel.locator(`.ant-picker-cell[title="${escapedTitle}"]:not(.ant-picker-cell-disabled) .ant-picker-cell-inner`).first());
    candidates.push(panel.locator(`[title="${escapedTitle}"]:not(.ant-picker-cell-disabled):not(.ant-calendar-disabled-cell)`).first());
  }

  const exactDay = new RegExp(`^\\s*${targetDate.day}\\s*$`);
  candidates.push(
    panel
      .locator(
        [
          'td:not(.ant-calendar-last-month-cell):not(.ant-calendar-next-month-btn-day):not(.ant-calendar-disabled-cell) .ant-calendar-date',
...
async function clickAntdDateCell(page, panel, targetDate) {
  const candidates = buildAntdDateCellCandidates(panel, targetDate);
  for (const cell of candidates) {
    const count = await cell.count().catch(() => 0);
    if (!count || !(await locatorVisible(cell, 180))) continue;
...
async function openAntdDatePickerPanelForInput(page, input, timeoutMs) {
  await input.click({ force: true }).catch(() => {});
  let panel = await waitForVisibleAntdDatePickerPanel(page, timeoutMs);
  if (panel) return panel;
  await dispatchMouseDown(input).catch(() => {});
  panel = await waitForVisibleAntdDatePickerPanel(page, Math.min(800, Number(timeoutMs || 1200)));
  return panel;
}

async function fillAntdDateTime(page, row, options) {
  const value = normalizeVisibleText(options?.value || options?.dateTime || options?.text || '');
  if (!value) throw new Error('fillAntdDateTime 需要提供 options.value');

  const timeoutMs = Math.max(500, Number(options?.timeoutMs || 8000));
  const pickerTimeoutMs = Math.max(150, Number(options?.pickerTimeoutMs || 1200));
  const input = isLocatorLike(options?.input) ? options.input : row.locator('input:visible').first();
  const targetDate = extractDateParts(value);

  await input.scrollIntoViewIfNeeded().catch(() => {});
  await input.waitFor({ state: 'visible', timeout: timeoutMs });

  if (!(await isInputReadonlyOrDisabled(input))) {
    try {
      await input.fill(value, { timeout: Math.min(timeoutMs, 1800) });
      await input.press('Enter').catch(() => {});
      const currentValue = normalizeVisibleText(await readInputValue(input));
      if (dateInputMatchesTarget(currentValue, value, options)) {
        emitLog('info', 'ant-date input filled', { strategy: 'editable-input', value, currentValue });
        return input;
      }
      emitLog('warn', 'ant-date editable fill did not match target', { value, currentValue });
    } catch {
      // Continue with picker and DOM-event fallbacks below.
    }
  }

  let panel = await openAntdDatePickerPanelForInput(page, input, pickerTimeoutMs);

  if (panel) {
    const popupInput = await pickVisibleLocator(buildAntdDatePickerPopupInputCandidates(panel), 400);
    if (popupInput) {
      await popupInput.click({ force: true }).catch(() => {});
      let filledInPopup = false;
      if (!(await isInputReadonlyOrDisabled(popupInput))) {
        try {
          await popupInput.fill(value, { timeout: 1200 });
          filledInPopup = true;
        } catch {
          filledInPopup = false;
        }
      ...
      if (!filledInPopup) {
        const setResult = await setInputValueWithEvents(popupInput, value);
        filledInPopup = setResult.ok;
      }

      await popupInput.press('Enter').catch(() => {});
      await page.waitForTimeout(Number(options?.postInputSettleMs || 120));
      await clickAntdDatePickerOkIfPresent(page, panel, Number(options?.okTimeoutMs || 1000)).catch(() => false);
      await waitForAntdDatePickerPanelToClose(page, 800).catch(() => false);

      const currentValue = normalizeVisibleText(await readInputValue(input));
      if (dateInputMatchesTarget(currentValue, value, options)) {
        emitLog('info', 'ant-date input filled', { strategy: 'picker-input', value, currentValue });
        return input;
      }

      emitLog('warn', 'ant-date picker input did not match target', { value, currentValue });
      if (filledInPopup && targetDate) {
        panel = await openAntdDatePickerPanelForInput(page, input, pickerTimeoutMs);
        if (panel && (await clickAntdDateCell(page, panel, targetDate))) {
          await clickAntdDatePickerOkIfPresent(page, panel, Number(options?.okTimeoutMs || 1000)).catch(() => false);
          await waitForAntdDatePickerPanelToClose(page, 800).catch(() => false);
          const selectedValue = normalizeVisibleText(await readInputValue(input));
          if (dateInputMatchesTarget(selectedValue, value, options)) {
            emitLog('info', 'ant-date input filled', { strategy: 'date-cell', value, currentValue: selectedValue });
            return input;
          }
          emitLog('warn', 'ant-date cell selection did not match target', { value, currentValue: selectedValue });
        }
      }

      if (filledInPopup && options?.allowNativeFallback === true) {
        await setInputValueWithEvents(input, value);
        const syncedValue = normalizeVisibleText(await readInputValue(input));
        if (dateInputMatchesTarget(syncedValue, value, options)) {
          emitLog('warn', 'ant-date input filled via native sync fallback', { value, currentValue: syncedValue });
          return input;
        }
      }
    }
  ...
  const result = await setInputValueWithEvents(input, value);
  await input.press('Enter').catch(() => {});
  await page.keyboard.press('Enter').catch(() => {});
  if (!result.ok) {
    throw new Error('未能写入 Ant Design 日期时间输入框');
  }

  const fallbackValue = normalizeVisibleText(await readInputValue(input));
  if (dateInputMatchesTarget(fallbackValue, value, options)) {
    emitLog('warn', 'ant-date input filled via native events fallback', { value, currentValue: fallbackValue });
    return input;
  }

  throw new Error(`Ant Design 日期时间输入未匹配目标值: target=${value}; current=${fallbackValue || '(empty)'}`);
}
...
function buildAntdOptionCandidates(dropdown, label, options) {
  ...
  if (!label && options?.pickFirstSearchMatch && searchText) {
    const searchTextPattern = new RegExp(escapeRegExp(searchText), 'i');
    const candidates = [];
    ...
    candidates.push(
      dropdown
        .locator('.ant-select-item-option-content, .ant-select-item-option, .ant-select-dropdown-menu-item, [role="option"], [role="treeitem"]')
        .filter({ hasText: searchTextPattern })
        .first()
    );
...
async function selectAntdOption(page, row, options) {
  ...
  const pickFirstSearchMatch = !label && Boolean(options?.pickFirstSearchMatch) && Boolean(searchText);
  const selectionTarget = label || searchText;

  if (!label && !pickFirstSearchMatch) {
    throw new Error('缺少下拉选项 label；若只提供 searchText，需同时设置 pickFirstSearchMatch');
  }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      const inlineSelection = label ? await trySelectInlineEnumOption(page, row, label) : null;
      if (inlineSelection) {
    ...
    if (searchText && searchInput) {
      emitLog('debug', 'ant-select search filled', { label: selectionTarget, searchText, attempt: attempt + 1 });
    ...
    if (await clickAntdOption(candidates)) {
      ...
      emitLog('info', 'ant-select option selected', {
        label: selectionTarget,
        attempt: attempt + 1,
        ...(pickFirstSearchMatch ? { strategy: 'first-search-match' } : {}),
      });
      return dropdown;
    }
    ...
    emitLog('warn', 'ant-select option not found, retrying', { label: selectionTarget, attempt: attempt + 1 });
  }

  throw new Error(`${pickFirstSearchMatch ? '未找到匹配下拉选项' : '未找到下拉选项'}：${selectionTarget}`);
}

function buildVisibleAntdFilterRootCandidates(page, options) {
  const candidates = [];
  const searchButton = isLocatorLike(options?.searchButton) ? options.searchButton : null;

  if (isLocatorLike(options?.filterRoot)) {
    candidates.push(options.filterRoot);
  }
  if (isLocatorLike(options?.scope) && options.scope !== options.filterRoot) {
    candidates.push(options.scope);
  }
  ...
  if (searchButton) {
    candidates.push(
      page
        .locator(
          '.ant-form:visible, form:visible, .sourceSearch:visible, .search:visible, .filter:visible, [class*="search"]:visible, [class*="filter"]:visible, .ant-card:visible'
        )
        .filter({ has: searchButton })
    );
  }
...
function buildVisibleAntdFilterSourceCandidates(page, filterRoot, options) {
  const fieldLabelPattern = options?.fieldLabelPattern instanceof RegExp ? options.fieldLabelPattern : /订单状态|入账状态/;
  const pageSelects = page.locator('.ant-select:visible');
  const root = isLocatorLike(filterRoot) ? filterRoot : page.locator('body');
  const candidates = [isLocatorLike(options?.filterSource) ? options.filterSource : null];

  candidates.push(
    root.locator('.ant-form-item:visible').filter({ hasText: fieldLabelPattern }),
    page.locator('.ant-form-item:visible').filter({ hasText: fieldLabelPattern }),
    root.locator('.ant-form-item:visible').filter({ has: root.locator('.ant-select:visible') }),
    page.locator('.ant-form-item:visible').filter({ has: page.locator('.ant-select:visible') }),
    root.locator('.ant-select:visible'),
    pageSelects
  );
...
async function waitForDeterministicVisibleFilterResults(resultScope, options) {
  const rows = resultScope.locator(String(options?.rowSelector || '.ant-table-tbody tr[data-row-key]:visible'));
  const placeholder = resultScope.locator(String(options?.placeholderSelector || '.ant-table-placeholder:visible')).first();
  const timeoutMs = Math.max(1200, Number(options?.resultTimeoutMs || 15000));
  const deadline = Date.now() + timeoutMs;

  while (Date.now() < deadline) {
    const rowCount = await rows.count().catch(() => 0);
    if (rowCount > 0 && (await locatorVisible(rows.first(), 250))) {
      return {
        rows,
        placeholder,
        rowCount,
    ...
    if (await locatorVisible(placeholder, 250)) {
      return {
        rows,
        placeholder,
        rowCount: 0,
  ...
  const rowCount = await rows.count().catch(() => 0);
  const placeholderVisible = await locatorVisible(placeholder, 250);
  if (rowCount > 0 || placeholderVisible) {
    return {
      rows,
      placeholder,
      rowCount,
  ...
  throw new Error('筛选后结果区未收敛');
}

async function applyDeterministicVisibleAntdFilter(page, options) {
  const label = String(options?.label || '').trim();
  if (!label) {
    throw new Error('applyDeterministicVisibleAntdFilter 缺少 label');
  }

  const expandButton =
    isLocatorLike(options?.expandButton) ? options.expandButton : page.getByRole('button', { name: /展\s*开/i }).first();
  if (expandButton && (await locatorVisible(expandButton, Number(options?.expandButtonTimeoutMs || 500)))) {
    await expandButton.scrollIntoViewIfNeeded().catch(() => {});
    await expandButton.click({ force: true }).catch(() => {});
    await page.waitForTimeout(Number(options?.postExpandSettleMs || 180));
  }

  const searchButton = await pickVisiblePrimaryLookupLocator(
    buildPrimaryLookupSearchButtonCandidates(page, {
      scope: options?.scope,
      searchScope: options?.searchScope,
      searchButton: options?.searchButton,
    }),
    Math.max(300, Number(options?.searchButtonTimeoutMs || 1200))
  );
  if (!searchButton) {
    throw new Error('未找到可见搜索按钮');
  }

  const filterRoot = await pickVisiblePrimaryLookupLocator(
    buildVisibleAntdFilterRootCandidates(page, {
      ...options,
      searchButton,
    }),
    Math.max(300, Number(options?.filterRootTimeoutMs || 800))
  );
  if (!filterRoot) {
    throw new Error('未找到可见筛选容器');
  }

  const filterSource = await pickVisiblePrimaryLookupLocator(
    buildVisibleAntdFilterSourceCandidates(page, filterRoot, options),
    Math.max(300, Number(options?.filterSourceTimeoutMs || 900))
  );
  if (!filterSource) {
    throw new Error('未找到可用的状态筛选控件');
  }

  await selectAntdOption(page, filterSource, {
  ...
  const listResponseOptions =
    options?.listResponse && typeof options.listResponse === 'object' && typeof options.listResponse.urlIncludes === 'string'
      ? {
          ...
          timeoutMs: Math.max(600, Number(options.listResponse.timeoutMs || options?.listResponseTimeoutMs || 5000)),
        }
      : null;
  const responsePromise = listResponseOptions ? waitForApiResponse(page, listResponseOptions).catch(() => null) : null;
  ...
  const resultScope = isLocatorLike(options?.resultScope)
    ? options.resultScope
    : isLocatorLike(options?.scope)
    ? options.scope
    : page;
  const busyState = await waitForBusyIndicatorsToSettle(resultScope, {
    timeoutMs: Math.max(600, Number(options?.busyTimeoutMs || 2200)),
  ...
  }).catch(() => ({ busySeen: false, timedOut: false }));
  const resultState = await waitForDeterministicVisibleFilterResults(resultScope, options);

  return {
    filteredBy: String(options?.summary || label).trim() || label,
    searchTriggered: true,
    listResponseObserved: Boolean(response),
    resultRowCount: Number(resultState.rowCount || 0),
    placeholderVisible: Boolean(resultState.placeholderVisible),
    busyTimedOut: Boolean(busyState?.timedOut),
  };
}
...
function buildPrimaryLookupInputCandidates(page, options) {
  const candidates = [isLocatorLike(options?.keywordInput) ? options.keywordInput : null];
  const roots = buildPrimaryLookupScopeCandidates(page, options);

  for (const root of roots) {
    candidates.push(
      root.locator('#businessList_keywords:visible'),
      root.locator('#service-data-item_keyWord:visible'),
      root.locator('#form_in_modal_testKeyWord:visible'),
      root.locator(
        'input[id*="testKeyWord"]:visible, input[name*="testKeyWord"]:visible, input[id*="keyWord"]:visible, input[name*="keyWord"]:visible'
      ),
      root.locator('input[id*="keyword"]:visible, input[name*="keyword"]:visible'),
      root.locator(
        [
          'input[placeholder*="请输入关键词"]:visible',
          'input[placeholder*="请输入关键字"]:visible',
          'input[placeholder*="商机ID"]:visible',
          'input[placeholder*="订单ID"]:visible',
          'input[placeholder*="联系人名称"]:visible',
          'input[placeholder*="企业名称"]:visible',
          'input[placeholder*="手机号"]:visible',
          'input[placeholder*="关键字"]:visible',
          'input[placeholder*="关键词"]:visible',
          'input[placeholder*="搜索"]:visible',
          'input[placeholder*="查询"]:visible',
          'input[placeholder*="businessId"]:visible',
          'input[placeholder*="orderId"]:visible',
        ].join(', ')
      ),
      ...
      root.locator(
        '.sourceSearch input:visible, .search input:visible, .filter input:visible, [class*="search"] input:visible, [class*="filter"] input:visible'
      )
    );
  }
...
function buildPrimaryLookupSearchButtonCandidates(page, options) {
  const candidates = [isLocatorLike(options?.searchButton) ? options.searchButton : null];
  const roots = buildPrimaryLookupScopeCandidates(page, options);

  for (const root of roots) {
    candidates.push(root.getByRole('button', { name: /搜\s*索|查\s*询|筛\s*选/i }));
  }

  return candidates.filter(Boolean);
}

function buildPrimaryLookupScopeCandidates(page, options) {
  const roots = [];
  const appendScope = (scope, includeFormContainers) => {
    if (!scope || typeof scope.locator !== 'function') return;

    roots.push(
      scope.locator(
        includeFormContainers
          ? '.sourceSearch, .search, .filter, [class*="search"], [class*="filter"], .ant-form, form'
          : '.sourceSearch, .search, .filter, [class*="search"], [class*="filter"]'
      )
    );
    roots.push(scope);
  };

  if (isLocatorLike(options?.searchScope)) {
    appendScope(options.searchScope, true);
  }

  if (isLocatorLike(options?.scope)) {
    appendScope(options.scope, true);
  }

  if (isLocatorLike(options?.table) && options.table !== options.scope) {
    appendScope(options.table, false);
  }

  appendScope(page, false);

  return roots.filter(Boolean);
}

async function pickVisiblePrimaryLookupLocator(candidates, timeout) {
  const candidateTimeout = timeout || 300;
  const maxMatchesPerCandidate = 8;

  for (const locator of candidates) {
    if (!locator || typeof locator.count !== 'function' || typeof locator.nth !== 'function') {
      continue;
    }

    const count = await locator.count().catch(() => 0);
    const matchCount = Math.min(count, maxMatchesPerCandidate);
    for (let index = 0; index < matchCount; index += 1) {
      const candidate = locator.nth(index);
      if (await locatorVisible(candidate, candidateTimeout)) {
        return candidate;
      }
    }
  }
...
async function waitForPrimaryLookupSurface(page, options) {
  ...
  while (Date.now() < deadline) {
    const ready = await pickVisiblePrimaryLookupLocator(candidates, 250);
    if (ready) return true;
    await delay(120);
  }
...
async function resolvePrimaryRecord(page, options) {
  ...
  for (let attemptIndex = 1; attemptIndex <= maxLookupAttempts; attemptIndex += 1) {
    ...
    const keywordInput = await pickVisiblePrimaryLookupLocator(buildPrimaryLookupInputCandidates(page, options), inputTimeoutMs);
    if (!keywordInput) {
