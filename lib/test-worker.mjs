...
async function fillAntdDateTime(page, row, options) {
  ...
  if (!(await isInputReadonlyOrDisabled(input))) {
    try {
      ...
      emitLog('info', 'ant-date input filled', { strategy: 'editable-input', value });
      return input;
    } catch {
  ...
  let panel = await waitForVisibleAntdDatePickerPanel(page, pickerTimeoutMs);
  if (!panel) {
    await dispatchMouseDown(input).catch(() => {});
    panel = await waitForVisibleAntdDatePickerPanel(page, Math.min(800, pickerTimeoutMs));
  }
  ...
  if (panel) {
    ...
    if (popupInput) {
      ...
      if (filledInPopup) {
        ...
        emitLog('info', 'ant-date input filled', { strategy: 'picker-input-native-sync', value });
        return input;
      }
    }
  }
  ...
  emitLog('warn', 'ant-date input filled via native events fallback', { value });
  return input;
}
