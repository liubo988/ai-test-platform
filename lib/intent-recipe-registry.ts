...
const BUILTIN_INTENT_RECIPES: IntentRecipe[] = [
  ...
  {
    version: 1,
    slug: 'document.project-knowledge-import-preview',
    title: '项目知识文档导入后预览校验',
    description: '针对当前平台项目知识文档 UI，固定“导入知识文档 -> 当前预览 -> 文档块正文锚点”验收链路。',
    matchers: {
    ...
    requiredContext: [
      '候选证据必须来自 traffic-quality source=real_click 的 document-like 样本。',
      '目标入口为当前平台 `/projects/:projectUid?intentView=knowledge`。',
      '文档名称需要带时间戳或稳定唯一后缀，避免复用旧文档造成假阳性。',
      '正文必须包含可搜索锚点，验收时必须进入当前预览/文档块区域校验。',
    ],
    executorPlan: [
      '进入项目页后点击“需求编排”，再进入“知识文档”视图；不要跳到外部 OCR 或跨系统文档站点。',
      '填写“知识文档名称 / 知识来源路径 / 知识文档内容”，点击“导入知识”并等待导入接口或 UI 收敛。',
      '导入内容至少包含一个本次唯一正文锚点，例如“真实 document-like real_click 采集锚点”。',
    ],
    verifierPlan: [
      '确认“知识文档已导入”或等价导入成功信号出现。',
      '确认“当前预览”已切换到本次唯一文档名称，而不是历史文档。',
      '在文档块预览区搜索正文锚点并断言锚点出现在预览渲染结果中；不能只断言 textarea 原文。',
    ],
    knownPitfalls: [
      '不要把“参考知识文档执行业务流”的 reference-only 请求当成 document family 治理证据。',
      '不要混入 benchmark_rerun、replay 或 draft_import 作为真实点击分母。',
      '不要以 OCR-first 或截图识别替代当前平台知识文档 UI 的导入/预览验证。',
    ],
    successRate: 100,
    lastVerifiedAt: '2026-05-08T07:57:47.372Z',
  },
  {
    version: 1,
    slug: 'document.project-knowledge-search-open-preview',
    title: '项目知识文档搜索打开预览校验',
    description: '针对当前平台项目知识文档 UI，固定“知识目录打开文档 -> 当前预览 -> 搜索文档块正文锚点”验收链路。',
    matchers: {
      targetUrlIncludes: ['/projects/', 'intentview=knowledge'],
      summaryIncludes: [
        '知识文档',
    ...
    requiredContext: [
      '候选证据必须来自 traffic-quality source=real_click 的 document-like 样本。',
      '目标入口为当前平台 `/projects/:projectUid?intentView=knowledge`。',
      '样本可通过 fixture setup 准备唯一知识文档，但验收动作必须在 UI 知识目录和文档块预览区完成。',
      '正文必须包含可搜索锚点，验收时必须打开目标文档并搜索文档块。',
    ],
    executorPlan: [
      '进入项目页后点击“需求编排”，再进入“知识文档”视图；不要跳到外部 OCR 或跨系统文档站点。',
      '在知识目录中定位目标文档并点击“预览”，确保当前预览切换到目标文档。',
      '在“搜索文档块”输入本次唯一正文锚点，并等待过滤后的文档块结果稳定。',
    ],
    verifierPlan: [
      '确认“当前预览”已切换到目标文档名称，而不是历史文档。',
      '确认“搜索文档块”输入后，文档块预览区展示本次唯一正文锚点。',
      '不能只断言目录中存在文档名称，必须校验预览内容。',
    ],
    knownPitfalls: [
      '不要把 fixture setup 的 API 写入当成最终验收；最终证据必须来自 UI 打开和预览搜索。',
      '不要把“参考知识文档执行业务流”的 reference-only 请求当成 document family 治理证据。',
      '不要混入 benchmark_rerun、replay 或 draft_import 作为真实点击分母。',
    ],
    successRate: 100,
    lastVerifiedAt: '2026-05-08T09:43:46.059Z',
  },
  {
    version: 1,
    slug: 'document.project-knowledge-edit-save-preview',
    title: '项目知识文档编辑保存预览校验',
    description: '针对当前平台项目知识文档 UI，固定“同名文档编辑保存 -> 当前预览 -> 更新正文锚点”验收链路。',
    matchers: {
      targetUrlIncludes: ['/projects/', 'intentview=knowledge'],
      summaryIncludes: [
        '知识文档',
    ...
    requiredContext: [
      '候选证据必须来自 traffic-quality source=real_click 的 document-like 样本。',
      '目标入口为当前平台 `/projects/:projectUid?intentView=knowledge`。',
      '样本可通过 fixture setup 准备唯一已有知识文档，但编辑保存动作必须经由当前平台知识文档 UI 完成。',
      '更新后的正文必须包含本次唯一锚点，验收时必须校验当前预览和文档块更新结果。',
    ],
    executorPlan: [
      '进入项目页后点击“需求编排”，再进入“知识文档”视图；不要跳到外部 OCR 或跨系统文档站点。',
      '先在知识目录中打开目标文档预览，确认原始正文锚点可见。',
      '用同名文档填写更新后的来源路径和知识内容，点击“导入知识”完成整篇替换保存。',
    ],
    verifierPlan: [
      '确认“当前预览”仍指向目标文档名称，而不是新建了另一篇文档。',
      '搜索旧正文锚点时应出现无匹配结果，证明旧切块已被替换。',
      '搜索更新后的正文锚点时，文档块预览区必须展示更新内容；不能只断言 textarea 或 toast。',
    ],
    knownPitfalls: [
      '不要把 fixture setup 的 API 写入当成最终验收；最终证据必须来自 UI 编辑保存和预览搜索。',
      '不要把“参考知识文档执行业务流”的 reference-only 请求当成 document family 治理证据。',
      '不要混入 benchmark_rerun、replay 或 draft_import 作为真实点击分母。',
    ],
    successRate: 100,
  ...
  {
    version: 1,
    slug: 'document.project-knowledge-archive-restore-preview',
    title: '项目知识文档归档恢复预览校验',
    description: '针对当前平台项目知识文档 UI，固定“知识目录归档 -> 恢复 -> 重新预览 -> 文档块正文锚点”验收链路。',
    matchers: {
      targetUrlIncludes: ['/projects/', 'intentview=knowledge'],
      summaryIncludes: [
        '知识文档',
    ...
    requiredContext: [
      '候选证据必须来自 traffic-quality source=real_click 的 document-like 样本。',
      '目标入口为当前平台 `/projects/:projectUid?intentView=knowledge`。',
      '样本可通过 fixture setup 准备唯一已有知识文档，但归档、恢复和恢复后预览动作必须经由当前平台知识文档 UI 完成。',
      '正文必须包含本次唯一锚点，验收时必须校验恢复后预览区仍可渲染该锚点。',
    ],
    executorPlan: [
      '进入项目页后点击“需求编排”，再进入“知识文档”视图；不要跳到外部 OCR 或跨系统文档站点。',
      '先在知识目录中打开目标文档预览，确认正文锚点可见。',
      '点击目标文档“归档”并接受确认弹窗，等待目标卡片出现“已归档”状态。',
      '点击目标文档“恢复”，等待目标卡片重新显示“归档”按钮。',
    ],
    verifierPlan: [
      '确认归档成功信号和“已归档”状态都出现在目标文档卡片上。',
      '确认恢复成功信号出现，且目标文档恢复到可再次归档状态。',
      '恢复后重新点击“预览”，在文档块预览区搜索正文锚点并断言渲染结果可见；不能只断言 API 或 toast。',
    ],
    knownPitfalls: [
      '不要把 fixture setup 的 API 写入当成最终验收；最终证据必须来自 UI 归档、恢复和恢复后预览。',
      '不要把归档/恢复 toast 直接当最终成功，必须校验目标文档状态和恢复后的文档块预览。',
      '不要混入 benchmark_rerun、replay 或 draft_import 作为真实点击分母。',
    ],
    successRate: 100,
    lastVerifiedAt: '2026-05-09T02:40:00.000Z',
  },
  {
    version: 1,
    slug: 'document.project-knowledge-derive-capability-preview',
    title: '项目知识文档自动沉淀能力目录校验',
    description: '针对当前平台项目知识文档 UI，固定“知识目录打开文档 -> 当前预览 -> 自动沉淀能力 -> 能力目录知识提炼验收”链路。',
    matchers: {
      targetUrlIncludes: ['/projects/', 'intentview=knowledge'],
      summaryIncludes: [
        '知识文档',
    ...
    requiredContext: [
      '候选证据必须来自 traffic-quality source=real_click 的 document-like 样本。',
      '目标入口为当前平台 `/projects/:projectUid?intentView=knowledge`。',
      '样本可通过 fixture setup 准备唯一已有知识文档，但自动沉淀和能力目录验收必须经由当前平台 UI 完成。',
      '文档内容需要产生本次唯一能力名称，避免复用历史“进入商机列表页”之类能力造成假阳性。',
    ],
    executorPlan: [
      '进入项目页后点击“需求编排”，再进入“知识文档”视图；不要跳到外部 OCR 或跨系统文档站点。',
      '先在知识目录中打开目标文档预览，确认正文锚点可见。',
      '点击文档块预览区的“自动沉淀能力”，等待“已沉淀 N 条能力”成功信号。',
      '进入能力目录后用“搜索稳定能力”筛选本次唯一能力名称。',
    ],
    verifierPlan: [
      '确认“当前预览”已切换到目标文档名称，而不是历史文档。',
      '确认自动沉淀提示为“已沉淀 N 条能力”，不能把“没有新增可沉淀能力”当通过。',
      '确认能力目录中出现本次唯一“商机列表按采集手机号<timestamp>检索”能力。',
      '确认该能力展示“知识提炼”验证状态。',
    ],
    knownPitfalls: [
      '不要把 fixture setup 的 API 写入当成最终验收；最终证据必须来自 UI 自动沉淀和能力目录。',
      '不要用历史已有能力或非唯一能力名称作为最终证据。',
      '不要混入 benchmark_rerun、replay 或 draft_import 作为真实点击分母。',
    ],
    successRate: 100,
    lastVerifiedAt: '2026-05-09T03:10:00.000Z',
  },
  {
    version: 1,
