...
## 标题
- 下一阶段第五刀：targeted top-up / assert_extract_ui 第 1 条 admissible sample
...
- 当前 3 个 insufficient cases 的真实 fixed-slice 状态已知：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui`: `runCount=0 / gap=3`
  - `eval_complex_enterprise_flow_scenario_ui_extract`: `runCount=0 / gap=3`
  - `eval_complex_enterprise_flow_scenario_ui_assert_extract`: `runCount=1 / gap=2`
- 上一轮 fidelity 诊断已确认：
  - low-pass corpus 的 request 1 是当前唯一已被 repo-native 历史证据证明能命中 `assert_extract_ui` 的 tracked request
  - low-pass request 2 / 3 在当前 post-slice 下都会漂到 `ui_extract_assert`
  - 因此本轮不能继续整包 low-pass，只能缩成 `assert_extract_ui` 的第 1 条 targeted sample

## 本轮目标
- 只做“第五刀 targeted top-up：assert_extract_ui 第 1 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui` 从 `runCount=0` 变成 `runCount=1`
...
- [ ] 若满足上述，只宣布“补入第 1 条 admissible sample 成功”，不宣布 recovery、freeze、第六刀或 Phase 5
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / assert_extract_ui 第 1 条 sample
- 本轮完成后准备回写：roadmap 最新一条进度更新

## 计划修改点
- 固定为什么当前仍是第五刀：因为第五刀还没有恢复成 official compare clean，也没有 closure freeze。
- 固定为什么这轮只能打 `assert_extract_ui` 第 1 条 sample：因为它当前 `gap=3`、不确定性最高，且 low-pass request 1 是唯一已证明可命中的 tracked request。
- 固定为什么不能继续整包 low-pass：因为 request 2 / 3 已被当前 post-slice evidence 证明会漂到 `ui_extract_assert`。
- 固定为什么这轮不能直接 freeze：因为本轮只是补第 1 条 admissible sample，不是 recovery 收口，更不是 closure freeze。
...
- 如果 replay 证明新 run 漂到别的 case，本轮必须立即停止，不能继续第 2 条 sample。
- 本轮不回答 recovery / compare / freeze，只回答“第 1 条 targeted sample 是否 admissible 且落点正确”。
