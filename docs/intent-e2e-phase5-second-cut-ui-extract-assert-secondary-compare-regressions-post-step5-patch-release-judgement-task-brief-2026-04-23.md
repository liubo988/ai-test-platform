...
## 标题
- Phase 5 / 第二刀：secondary compare regressions post-step5-patch release judgement

## 背景
- 当前仍是 `Phase 5 / 第二刀`，不是 freeze，也不是第三刀。
- 上一轮已完成：
  - `secondary compare regressions list shared-path step-5 detail-evidence chain code-recovery`
...
- 因为 touched shared path = 是：
  ...
  - 之前 sibling dedicated probe 结果也不得直接作为当前放行证据
- 但这次 patch 只落在 generator Step 5 builder / sanitizer：
  - 没改 `lib/test-worker.mjs`
  - 没改 `lib/ai/intent-e2e-service.ts`
  - 没改 `scripts/intent-e2e-benchmark.ts`
  - 没改 harness / corpus
- 当前核心问题不是是否继续 code diagnosis，而是是否已经满足重新启动 probes 的 release judgement 条件。

## 本轮目标
- 只读判断：这次 shared-path Step 5 patch 之后，secondary compare regressions 是否允许重启 shared-path modal/list proof 与后续 sibling probes。
...
- `reports/intent-e2e/projects/proj_default/intent-e2e.benchmark-reports/2026-04-23T03-40-16-522Z-family-modal_or_drawer_save-fresh-rerun.json`
...
- `reports/intent-e2e/runs/intent-run-29026620-15ed-4923-8679-0898ff3feff5/attempt-1-response-summary.json`
- `reports/intent-e2e/runs/intent-run-5ff6bde8-c34a-4e83-8cf8-c2fa4521d516/attempt-1-response-summary.json`
- `reports/intent-e2e/runs/intent-run-a816e5d3-d04a-43f6-ab2d-1706ea14311b/attempt-1-response-summary.json`

## Roadmap 对齐
- 当前阶段：`Phase 5 / 第二刀`
- 对应小步：secondary compare regressions post-step5-patch release judgement
...
## 固定结论
- `Q1`：因为这次 Step 5 shared-path patch 已落地，旧 shared-path modal/list proof 全部失效。
...
- `Q4`：exact command plan 继续沿用既定 cadence：
  ...
  - official compare
  - compare label 更新为 `phase5-second-cut-secondary-compare-regressions-post-step5-patch-current-2026-04-23`
- `Q5`：stop conditions 不需要新增或调整；继续沿用既定 gating 规则即可。

## 为什么不是 B / C
- 不是 `B`：
  - 当前没有新的只读 guard 需要先补
  - “旧 proof 失效需要重跑”属于 execution plan，而不是新的 blocker
- 不是 `C`：
  - 当前没有证据表明还要继续 code-recovery
  - 当前没有证据表明需要回到 harness / compare 口径
  - 上一轮 Step 5 patch 已通过 unit/build/build:web/boundaries/doc/roadmap 校验
