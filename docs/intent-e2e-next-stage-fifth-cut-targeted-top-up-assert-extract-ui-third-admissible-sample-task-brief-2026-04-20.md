...
## 标题
- 下一阶段第五刀：targeted top-up / assert_extract_ui 第 3 条 admissible sample
...
- 当前 fixed-slice 下目标 case 已补到：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui`
  - `runCount=2`
  - `sampleRunIds=["intent-run-8ecd92f9-c97e-4ce9-b3b4-92b5b7096956","intent-run-f307ad97-228e-4231-8cbf-bdfdd6ba7ed4"]`
  - 当前 gap 为 `1`
- 已确认的 corpus-to-case fidelity 仍不变：
  - low-pass corpus 的 request 1 是当前唯一已被 repo-native 历史证据证明能稳定命中 `assert_extract_ui` 的 tracked request
  - low-pass request 2 / 3 在当前 post-slice 下会漂到 `ui_extract_assert`
  - 因此本轮仍不能继续整包 low-pass，只能继续 `low-pass corpus + --max-requests 1`

## 本轮目标
- 只做“第五刀 targeted top-up：assert_extract_ui 第 3 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 记录：
  ...
  - `finishedAt`
  - `failureClass`
- [ ] `requestId` 必须是 `modal-phase2-assert-extract-ui-representative`
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui` 从 `runCount=2` 变成 `runCount=3`
  - 该 case 的 `sampleRunIds` 包含本轮新 `runId`
  - 该 case 的 `sampleRunIds` 仍包含 `intent-run-8ecd92f9-c97e-4ce9-b3b4-92b5b7096956`
  - 该 case 的 `sampleRunIds` 仍包含 `intent-run-f307ad97-228e-4231-8cbf-bdfdd6ba7ed4`
  - 本轮新 `runId` 没有漂到 `ui_extract_assert`
  - 本轮新 `runId` 没有漂到 `ui_extract`
  - 本轮新 `runId` 没有漂到 `ui_assert_extract`
...
- 会读：
  ...
  - `docs/intent-e2e-next-stage-fifth-cut-targeted-top-up-assert-extract-ui-second-admissible-sample-task-brief-2026-04-20.md`
- 会执行：
...
- 会改：
  - `docs/intent-e2e-next-stage-fifth-cut-targeted-top-up-assert-extract-ui-third-admissible-sample-task-brief-2026-04-20.md`
  - `docs/intent-e2e-high-success-roadmap-2026-03-20.md`
- 不会改：
  - `lib/**`
  - `scripts/**`
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / assert_extract_ui 第 3 条 sample
- 本轮完成后准备回写：roadmap 最新一条进度更新

## 计划修改点
- 固定为什么当前仍是第五刀：因为第五刀还没有恢复成 official compare clean，也没有 closure freeze。
- 固定为什么这轮只能打 `assert_extract_ui` 第 3 条 sample：因为它当前 gap=`1`，是最小、最确定、最接近跨过 `MIN_EVIDENCE_RUN_COUNT=3` 的 case。
- 固定为什么不能继续整包 low-pass：因为 request 2 / 3 已被当前 post-slice evidence 证明会漂到 `ui_extract_assert`。
- 固定为什么这轮不能直接 freeze：因为即使本轮成功把 `assert_extract_ui` 补满，`ui_assert_extract` 与 `ui_extract` 仍未补满，本轮也不做 compare。
...
- 如果 replay 证明新 run 漂到别的 case，本轮必须立即停止，不能切去别的 case。
