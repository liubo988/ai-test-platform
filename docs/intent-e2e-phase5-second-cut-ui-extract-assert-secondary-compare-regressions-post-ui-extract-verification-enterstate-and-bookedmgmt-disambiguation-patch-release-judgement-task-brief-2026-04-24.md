...
## 标题
- Phase 5 / 第二刀：secondary compare regressions post-`ui_extract` verification enterState-and-bookedMgmt-disambiguation patch release judgement

## 背景
- 当前仍是 `Phase 5 / 第二刀`，不是 freeze，也不是第三刀。
- 刚完成一轮最小 code-recovery：
  - verification `plan_step_2` payload 从字面“待申请”断言收口为 `enterState: 1`
  - verification bookedMgmt lookup 从 bare `[primaryOrderNo]` 收口为 disambiguated rowHasTexts + plan_step_6/7 row reuse
...
## Roadmap 对齐
- 当前阶段：`Phase 5 / 第二刀`
- 对应小步：secondary compare regressions post-`ui_extract` verification enterState-and-bookedMgmt-disambiguation patch release judgement
- 本轮完成后回写：roadmap 最新一条更新

## 计划修改点
- 审核这次 patch 的影响面，确认是否只触发 shared-path proof 失效而没有新增 blocker。
- 若 admissible，固定新的 rerun / replay / compare cadence 和 compare label。
