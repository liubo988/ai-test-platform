...
## 标题
- Phase 5 / 第五刀：ui_extract first admissible sample

## 背景
- `Phase 5 / 第四刀` 已正式收官。
- `Phase 5 / 第五刀：admissibility judgement` 预期判定：
  - 当前允许正式开启第五刀
...
## 本轮目标
- 验证当前 repo-native 状态下，`ui_extract` 是否能拿到第五刀的 first admissible sample。
...
- replay gate 通过后执行 official compare：
  - `phase5-fifth-cut-ui_extract-first-admissible-sample-current-2026-04-24`
...
- `npm run intent:benchmark:compare -- --project-uid proj_default --priority-scenario-family modal_or_drawer_save --proof-window non_weak --run-limit 200 --compared-label phase5-fifth-cut-ui_extract-first-admissible-sample-current-2026-04-24 --recipe-asset-input artifacts/intent-e2e-family-evidence/proj_default.project-recipes.json --json`
...
## 完成后动作
- 回写 roadmap
- 若第五刀首样本 clean，则继续判断是否直接进入第五刀收官 freeze
