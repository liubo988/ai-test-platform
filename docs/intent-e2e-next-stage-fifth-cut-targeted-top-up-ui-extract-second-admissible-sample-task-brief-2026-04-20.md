...
## 标题
- 下一阶段第五刀：targeted top-up / ui_extract 第 2 条 admissible sample
...
- 当前 fixed-slice 下 case 状态已知：
  ...
  - `eval_complex_enterprise_flow_scenario_ui_extract`
    - `runCount=1`
    - `sampleRunIds=["intent-run-21636080-0e6e-45aa-be69-26c7a3424c66"]`
    - 当前 gap 为 `2`
- 当前唯一 remaining insufficient case 仍是 `ui_extract`，所以本轮只能补它的第 2 条 admissible sample。
...
## 本轮目标
- 只做“第五刀 targeted top-up：ui_extract 第 2 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_ui_extract` 从 `runCount=1` 变成 `runCount=2`
  - 该 case 的 `sampleRunIds` 包含既有 `intent-run-21636080-0e6e-45aa-be69-26c7a3424c66`
...
- 会改：
  - `docs/intent-e2e-next-stage-fifth-cut-targeted-top-up-ui-extract-second-admissible-sample-task-brief-2026-04-20.md`
  - `docs/intent-e2e-high-success-roadmap-2026-03-20.md`
- 不会改：
  - `lib/**`
  - `scripts/**`
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / ui_extract 第 2 条 sample
...
- 固定为什么这轮不能 freeze：因为本轮只补 `ui_extract` 的第 2 条 sample，本轮也不做 compare。
