...
## 标题
- 下一阶段第五刀：targeted top-up / ui_assert_extract 第 2 条 admissible sample
...
- 当前 fixed-slice 下 case 状态已知：
  ...
  - `eval_complex_enterprise_flow_scenario_ui_assert_extract`
    - `runCount=1`
    - `sampleRunIds=["intent-run-f8666302-1f05-4bbb-b75b-01b61b6a8df7"]`
    - 当前 gap 为 `2`
  - `eval_complex_enterprise_flow_scenario_ui_extract`
    - `runCount=0`
    - 当前 gap 为 `3`
- 已确认的 corpus-to-case fidelity 仍不变：
  - `ui_assert_extract` 已有 dedicated corpus，可稳定命中目标 case
  - low-pass request 2 / 3 会漂到 `ui_extract_assert`
  - 因此本轮不应继续整包 low-pass，而应切到 dedicated `ui-assert-extract` corpus

## 本轮目标
- 只做“第五刀 targeted top-up：ui_assert_extract 第 2 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_ui_assert_extract` 从 `runCount=1` 变成 `runCount=2`
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / ui_assert_extract 第 2 条 sample
- 本轮完成后准备回写：roadmap 最新一条进度更新

## 计划修改点
- 固定为什么当前仍是第五刀：因为第五刀还没有恢复成 official compare clean，也没有 closure freeze。
- 固定为什么这轮应该切到 `ui_assert_extract`：因为 `assert_extract_ui` 已补满，`ui_assert_extract` 当前 gap=`2`，比 `ui_extract` 更接近跨过门槛，且已有 dedicated corpus，case fidelity 已锁定。
- 固定为什么不能继续整包 low-pass：因为 low-pass request 2 / 3 已被当前 post-slice evidence 证明会漂到 `ui_extract_assert`。
- 固定为什么这轮不能直接 freeze：因为即使本轮成功，`ui_assert_extract` 也只会从 `1 -> 2`，`ui_extract` 仍为 `0`，本轮也不做 compare。
...
- 如果 replay 证明新 run 漂到别的 case，本轮必须立即停止，不能继续第 3 条 sample，也不能切去 `ui_extract`。
