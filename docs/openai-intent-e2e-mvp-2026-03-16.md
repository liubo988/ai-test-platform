...
- 观察期默认看合并后的前 6 次终态运行，并用合并前最近 5 次终态运行做基线；当观察样本达到至少 3 次后，如果通过率跌到 35% 以下，或相对基线下滑达到 15 个点，就会自动标记为 `degraded`
- 被判定为 `degraded` 的新规则会在规划阶段自动大幅降权，并避免和已有 rollback risk 再重复扣分；如果顺利完成 6 次观察且未触发降级，则自动转为 `promoted`
- `IntentE2EWorkbench` 的“历史运行洞察”区域现在新增“新规则观察期”卡片，直接展示合并标题、基线通过率、观察期通过率、剩余样本数、建议动作和备份路径
- 如果某批新规则在观察期内已经明显拖低成功率，工作台会允许直接从观察期卡片一键回滚，不需要先跳去备份列表或审计列表手工定位
- generate / repair 阶段现在统一吃同一份规则表现反馈：稳定规则优先，观察期规则轻微降权，观察期已降级或已经命中过回滚候选的规则会被自动跳过
- 这一步真正解决的是“刚沉淀的新规则到底该不该立刻放量”这个问题：系统先用少量真实运行替你试错，再决定保留、转正还是降级
...
- 下一步建议优先做：把已转正且连续稳定的规则继续提炼成 starter recipe / runtime helper，进一步降低首轮接入成本，再补更多业务 helper
