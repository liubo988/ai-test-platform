...
## 标题
- 下一阶段第五刀：targeted top-up / assert_extract_ui 第 2 条 admissible sample
...
- 第 1 条 targeted sample 已补入：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui`
  - `runCount=1`
  - `sampleRunIds=["intent-run-f307ad97-228e-4231-8cbf-bdfdd6ba7ed4"]`
  - 当前 gap 仍为 `2`
- 已确认的 corpus-to-case fidelity 不变：
  - low-pass corpus 的 request 1 仍是当前唯一已被 repo-native 历史证据证明能命中 `assert_extract_ui` 的 tracked request
  - low-pass request 2 / 3 在当前 post-slice 下会漂到 `ui_extract_assert`
  - 因此本轮仍不能继续整包 low-pass，只能继续 `low-pass corpus + --max-requests 1`

## 本轮目标
- 只做“第五刀 targeted top-up：assert_extract_ui 第 2 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_assert_extract_ui` 从 `runCount=1` 变成 `runCount=2`
...
- [ ] 若满足上述，只宣布“补入第 2 条 admissible sample 成功”，不宣布 recovery、freeze、第六刀或 Phase 5
...
- 会读：
  ...
  - `docs/intent-e2e-next-stage-fifth-cut-targeted-top-up-assert-extract-ui-first-admissible-sample-task-brief-2026-04-20.md`
- 会执行：
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / assert_extract_ui 第 2 条 sample
- 本轮完成后准备回写：roadmap 最新一条进度更新

## 计划修改点
- 固定为什么当前仍是第五刀：因为第五刀还没有恢复成 official compare clean，也没有 closure freeze。
- 固定为什么这轮只能打 `assert_extract_ui` 第 2 条 sample：因为 `assert_extract_ui` 仍是 gap=`2`，且 low-pass request 1 仍是唯一已证明可命中的 tracked request。
- 固定为什么不能继续整包 low-pass：因为 request 2 / 3 已被当前 post-slice evidence 证明会漂到 `ui_extract_assert`。
- 固定为什么这轮不能直接 freeze：因为本轮只补第 2 条 admissible sample，不做 compare，也不判断 recovery，更不做 closure baseline freeze。
...
- 如果 replay 证明新 run 漂到别的 case，本轮必须立即停止，不能继续第 3 条 sample。
- 本轮不回答 recovery / compare / freeze，只回答“第 2 条 targeted sample 是否 admissible 且落点正确”。
