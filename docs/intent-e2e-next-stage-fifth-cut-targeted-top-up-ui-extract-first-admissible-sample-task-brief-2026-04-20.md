...
## 标题
- 下一阶段第五刀：targeted top-up / ui_extract 第 1 条 admissible sample
...
- 当前 fixed-slice 下 case 状态已知：
  ...
  - `eval_complex_enterprise_flow_scenario_ui_extract`
    ...
    - `sampleRunIds=[]`
    - 当前 gap 为 `3`
- 当前唯一 remaining insufficient case 就是 `ui_extract`。
- 已确认的 corpus-to-case fidelity 仍不变：
  - dedicated `ui-extract` corpus 可稳定命中目标 case
  - low-pass request 2 / 3 会漂到 `ui_extract_assert`
  - 因此本轮应切到 dedicated `ui-extract` corpus，而不是 low-pass

## 本轮目标
- 只做“第五刀 targeted top-up：ui_extract 第 1 条 admissible sample”。
- 只做：
  - brief
  - roadmap 回写
  - 1 条 targeted rerun
...
- [ ] 只读 fixed-slice replay 后：
  - `eval_complex_enterprise_flow_scenario_ui_extract` 从 `runCount=0` 变成 `runCount=1`
...
- 会改：
  - `docs/intent-e2e-next-stage-fifth-cut-targeted-top-up-ui-extract-first-admissible-sample-task-brief-2026-04-20.md`
  - `docs/intent-e2e-high-success-roadmap-2026-03-20.md`
- 不会改：
  - `lib/**`
  - `scripts/**`
...
## Roadmap 对齐
- 当前阶段：后续阶段 / 下一阶段第五刀
- 对应小步：targeted post-slice top-up / ui_extract 第 1 条 sample
- 本轮完成后准备回写：roadmap 最新一条进度更新

## 计划修改点
- 固定为什么当前仍是第五刀：因为第五刀还没有恢复成 official compare clean，也没有 closure freeze。
- 固定为什么这轮应该切到 `ui_extract`：因为 `assert_extract_ui` 与 `ui_assert_extract` 已补满，`ui_extract` 已经是唯一 remaining insufficient case。
- 固定为什么不能继续整包 low-pass：因为 low-pass request 2 / 3 已被当前 post-slice evidence 证明会漂到 `ui_extract_assert`。
- 固定为什么这轮不能直接 freeze：因为本轮只补 `ui_extract` 的第 1 条 sample，本轮也不做 compare。
