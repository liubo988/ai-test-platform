...
import {
  ...
  type IntentE2EFormalTaskRealClickSeedPlanItem,
} from '@/lib/intent-e2e-formal-task-seed-audit';
import { normalizeIntentE2EPriorityScenarioFamily, type IntentE2EPriorityScenarioFamily } from '@/lib/intent-e2e-priority-scenario-family';
...
const HELP_TEXT = `
Usage:
  npm run intent:formal-task-seed-runs -- [options]

Options:
  --base-url <url>                    Default: ${DEFAULT_BASE_URL}
  --project-uid <uid>                 Default: ${DEFAULT_PROJECT_UID}
  --actor-user-uid <uid>              Default: ${DEFAULT_ACTOR_USER_UID}
  --priority-scenario-family <name>   Optional tracked family filter.
  --document-only                     Only seed document-like formal tasks.
  --max-samples <n>                   Default: 5.
  --repeat <n>                        Default: 1.
...
function buildRunRequest(input: {
  projectUid: string;
  sample: IntentE2EFormalTaskRealClickSeedPlanItem;
...
async function enrichResultFromSnapshot(result: SeedRunResult): Promise<void> {
  ...
  result.matchedRuleTitles = uniqueStringArray(knowledge?.matchedRuleTitles);
  ...
  result.knowledgeHit = result.matchedRuleIds.length > 0;
  result.recipeHit = result.matchedRecipeSlugs.length > 0;
}
...
async function writeReport(input: {
  ...
  const jsonPath = path.join(reportDir, `intent-e2e.formal-task-real-click-seed-report.${timestamp}.json`);
  const mdPath = path.join(reportDir, `intent-e2e.formal-task-real-click-seed-report.${timestamp}.md`);
  const latestJsonPath = path.join(reportDir, 'intent-e2e.formal-task-real-click-seed-report.latest.json');
  const latestMdPath = path.join(reportDir, 'intent-e2e.formal-task-real-click-seed-report.latest.md');
  const report = {
    version: 1,
    generatedAt: input.generatedAt,
    projectUid: input.projectUid,
    sourcePolicy: 'formal_task_seed_to_real_click',
    denominatorPolicy:
      '这些请求通过 launch-decision / runs 入口产生 traffic-quality source=real_click 事件；正式任务执行历史本身仍只作为 seed/reference。',
    dryRun: input.dryRun,
    summary: {
      sampleCount: input.results.length,
      autoRunStarted: input.results.filter((item) => item.runId).length,
      terminalRuns: input.results.filter((item) => isTerminalStatus(item.status)).length,
      passedRuns: input.results.filter((item) => item.status === 'passed').length,
      failedRuns: input.results.filter((item) => item.status === 'failed').length,
      blockedRuns: input.results.filter((item) => item.status === 'blocked').length,
      timedOutRuns: input.results.filter((item) => item.timedOut).length,
      requestCount: input.results.length,
      knowledgeHitRuns: input.results.filter((item) => item.knowledgeHit).length,
      knowledgeHitRate: toPercent(input.results.filter((item) => item.knowledgeHit).length, input.results.length),
      recipeHitRuns: input.results.filter((item) => item.recipeHit).length,
      recipeHitRate: toPercent(input.results.filter((item) => item.recipeHit).length, input.results.length),
    },
    results: input.results,
    runs: input.results,
  };
  const lines = [
    '# Intent E2E Formal Task Real Click Seed Report',
    '',
    `- generatedAt: ${input.generatedAt}`,
    `- projectUid: ${input.projectUid}`,
    `- sourcePolicy: ${report.sourcePolicy}`,
    `- denominatorPolicy: ${report.denominatorPolicy}`,
    `- dryRun: ${input.dryRun ? 'yes' : 'no'}`,
    ...
    `- knowledgeHitRuns: ${report.summary.knowledgeHitRuns}`,
    `- knowledgeHitRate: ${report.summary.knowledgeHitRate}%`,
    `- recipeHitRuns: ${report.summary.recipeHitRuns}`,
    '',
    'sampleId | name | family | launchDecision | runId | status | knowledge | error',
    '--- | --- | --- | --- | --- | --- | --- | ---',
  ...
  await fsPromises.writeFile(mdPath, `${lines.join('\n')}\n`, 'utf8');
  await fsPromises.writeFile(latestJsonPath, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
  await fsPromises.writeFile(latestMdPath, `${lines.join('\n')}\n`, 'utf8');
...
async function main() {
  ...
  const family = normalizeIntentE2EPriorityScenarioFamily(readString(parsed.values['priority-scenario-family']));
  const maxSamples = readPositiveInt(parsed.values['max-samples'], 5);
  ...
  const tasks = await loadIntentE2EFormalTaskSeedAuditTasks({ projectUid });
  const plan = buildIntentE2EFormalTaskRealClickSeedPlan({
    projectUid,
    tasks,
    priorityScenarioFamily: family,
    documentOnly: Boolean(parsed.values['document-only']),
    maxSamples,
    repeat,
  });
  const results: SeedRunResult[] = plan.map((sample) => ({
    sampleId: sample.sampleId,
    configUid: sample.configUid,
    moduleUid: sample.moduleUid,
    name: sample.name,
    targetUrl: sample.targetUrl,
    priorityScenarioFamily: sample.priorityScenarioFamily,
    documentFamily: sample.documentFamily,
    launchDecision: '',
    launchReason: '',
    runId: '',
  ...
  console.log(
    `[formal-task-seed] start project=${projectUid} samples=${plan.length} family=${family || '-'} dryRun=${dryRun ? 'yes' : 'no'}`
  );

  for (let index = 0; index < plan.length; index += 1) {
    const sample = plan[index];
    const result = results[index];
    const request = buildRunRequest({ projectUid, sample });

    if (dryRun) {
      console.log(`[formal-task-seed] sample=${sample.sampleId} planned name=${sample.name}`);
      continue;
    }

    try {
      console.log(`[formal-task-seed] sample=${sample.sampleId} launch-decision`);
      ...
      if (result.launchDecision !== 'auto_run') {
        ...
        console.log(`[formal-task-seed] sample=${sample.sampleId} blocked decision=${result.launchDecision || '-'}`);
        continue;
      }

      console.log(`[formal-task-seed] sample=${sample.sampleId} start-run`);
      ...
      console.log(`[formal-task-seed] sample=${sample.sampleId} poll runId=${result.runId}`);
      ...
      console.log(`[formal-task-seed] sample=${sample.sampleId} done run=${result.runId || '-'} status=${result.status || '-'}`);
    } catch (error) {
      result.errorMessage = error instanceof Error ? error.message : String(error || '');
      console.log(`[formal-task-seed] sample=${sample.sampleId} error=${result.errorMessage}`);
    }
  }
  ...
  console.log(
    `[formal-task-seed] summary samples=${results.length} autoRun=${results.filter((item) => item.runId).length} terminal=${results.filter((item) => isTerminalStatus(item.status)).length} passed=${results.filter((item) => item.status === 'passed').length} failed=${results.filter((item) => item.status === 'failed').length} blocked=${results.filter((item) => item.status === 'blocked').length}`
  );
  console.log(`[formal-task-seed] report.json=${reportPaths.jsonPath}`);
  console.log(`[formal-task-seed] report.md=${reportPaths.mdPath}`);
}
